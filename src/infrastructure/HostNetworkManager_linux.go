...
func (h *HostNetworkManager) parseRule(rule domain.HostNetworkTrafficRule) []string {
	...
	if rule.Target != "" {
		...
		rulespec = append(rulespec, rule.Target)
	}
	return rulespec
}
...
func (h *HostNetworkManager) SaveConfiguration() error {
	...
	for _, table := range netfilterTables {
		...
		h.setConfigField(table, rules, &config)
	}
...
func (h *HostNetworkManager) setConfigField(table string, value []domain.HostNetworkTrafficRule, config *domain.HostNetworkConfig) {
	switch table {
	case "filter":
		config.TrafficRules.Filter = value
	case "nat":
		config.TrafficRules.NAT = value
	case "mangle":
		config.TrafficRules.Mangle = value
	case "raw":
		config.TrafficRules.Raw = value
	case "security":
		config.TrafficRules.Security = value
	}
	return
}

func (h *HostNetworkManager) getConfigField(table string, config domain.HostNetworkConfig) []domain.HostNetworkTrafficRule {
	switch table {
	case "filter":
...
func (h *HostNetworkManager) loadTrafficConfiguration(config domain.HostNetworkConfig) error {
	for _, table := range netfilterTables {
		...
		configField := h.getConfigField(table, config)
		if configField == nil {
